...
2. `.claude/skills/woozoo-phase-gates/SKILL.md`
3. `.claude/skills/woozoo-safety-boundaries/SKILL.md`
4. 작업이 금액·주문·원장·리스크를 건드리면 `.claude/skills/woozoo-financial-invariants/SKILL.md`
5. Binance 데이터가 관련되면 `.claude/skills/woozoo-binance-public-data/SKILL.md`
...
|----------|------|-----------|-------------|
| `woozoo-system-architect` | 구조, 신뢰 경계, API·이벤트·ERD, Phase 계획 | phase-gates, safety-boundaries | `_workspace/{phase}_woozoo-system-architect_{artifact}.md` |
| `woozoo-market-agent-builder` | 공개 데이터, Evidence, LangGraph 분석 조직 | binance-public-data, safety-boundaries | `_workspace/{phase}_woozoo-market-agent-builder_{artifact}.md` |
| `woozoo-risk-ledger-builder` | Risk, Paper Broker, 원장, 멱등성, 대조 | financial-invariants, safety-boundaries | `_workspace/{phase}_woozoo-risk-ledger-builder_{artifact}.md` |
| `woozoo-web-platform-builder` | Next.js, FastAPI 경계, 승인·Kill Switch UI, 운영 | phase-gates, safety-boundaries | `_workspace/{phase}_woozoo-web-platform-builder_{artifact}.md` |
...
1. `woozoo-system-architect`가 변경 계약과 성공 기준을 작성한다.
2. 제품 Phase 0에서는 market-agent, risk-ledger, web-platform builder가 각 도메인의 설계·테스트 초안을 병렬 작성한다. 테스트 매트릭스와 반례를 포함한 문서만 허용하며 실행 가능한 테스트 코드·fixture·scaffold는 만들지 않는다.
3. 그 초안이 모인 뒤 `woozoo-safety-qa-inspector`가 생산자와 역할을 분리한 내부 관점에서 교차 검증한다.
...
2. 구현 에이전트는 자신에게 필요한 전용 스킬과 `references/dev-rules.md`, `references/dev-rules.local.md`, `references/tdd-doctrine.md`를 따른다.
...
3. `external-review-loop`는 다른 엔진 리뷰에만 사용한다. 후보는 `select-project-reviewers.sh` 출력만 신뢰하며 Claude reviewer는 모든 런타임에서 비활성이다. Codex 러너 후보는 agy/Gemini뿐이고, Claude 러너는 외부 Codex와 agy/Gemini를 사용할 수 있다.
4. 외부 엔진이 없거나 실행에 실패하면 `external-review-unavailable`을 기록한다. safety QA나 같은 엔진 Codex 교차검토를 외부 PASS로 승격하지 않는다.
5. 모든 리뷰 지적은 실파일과 대조해 확인·부분 확인·이월·기각으로 판정한다. 합의만으로 확정하지 않는다.
6. 중대 작업의 외부 리뷰 루프는 신규 확인 0건이 2회 연속일 때 수렴으로 본다. 최대 3라운드다. 외부 리뷰어가 없으면 수렴을 허위 산출하지 않고 부재 상태로 끝낸다.
7. 개발 Phase 승인은 acceptance artifact digest에 결속한다. 승인 전에는 Phase를 올리거나 push하지 않는다.
8. Testnet 주문 승인은 별개다. Proposal hash, Risk Decision hash, 정확한 order preview, TTL과 일회성 nonce에 결속하며 한 번 사용하거나 만료되면 재사용하지 않는다.
