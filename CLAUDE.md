...
**트리거:** Woozoo의 설계, 구현, 테스트, Binance 데이터, 에이전트, Paper Broker, Risk Engine, 원장, 승인 UI, 운영 또는 이전 결과의 재실행·수정·보완 요청에는 `.claude/skills/woozoo-orchestrator/SKILL.md`를 사용하라. 단순 개념 질문은 직접 답할 수 있다.

**안전 포인터:** 모든 작업은 `.claude/skills/woozoo-safety-boundaries/SKILL.md`와 `.claude/skills/woozoo-phase-gates/SKILL.md`를 먼저 적용한다. Mainnet 비공개 주문, 실거래, 출금, 선물, 마진, 레버리지, 숏, 사람 승인 없는 외부 주문과 AI의 주문 도구 접근은 금지한다.
...
**Claude 어댑터:** 팀 도구가 가용하면 `.claude/agents/*.md`의 전문 에이전트를 팀원으로 사용하고, 작업 목록·메시지·`_workspace/` 산출물로 조율한다. 동시 실행은 기본 3개를 넘지 않는다.

**리뷰 정책:** Claude 자체를 일반 검토자로 사용하지 않는다. Claude Code 런타임에서 교차엔진 일반 검토가 필요하면 `external-review-loop`가 외부 `codex exec --sandbox read-only`를 호출하고, Codex 런타임에서는 Codex 전용 `woozoo-codex-cross-reviewer`가 내부 교차검토를 맡는다. 같은 엔진 Codex 검토는 외부 독립 리뷰가 아니며 safety QA를 대체하지 않는다.

**변경 이력:**

| 날짜 | 변경 내용 | 대상 | 사유 |
|------|-----------|------|------|
| 2026-07-19 | Claude 일반 검토를 제거하고 Codex 전용 내부 교차검토 역할·외부 Codex 호출 정책 추가 | 하네스 리뷰 경로 | 리뷰 엔진과 독립성 경계를 명시하고 Claude 자기검토 방지 |
