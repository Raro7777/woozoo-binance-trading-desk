...
- 테스트 명령, 권한, dependency, artifact upload와 제품 코드는 바뀌지 않았으므로 `c0baf4c` 제품 교차검토는 유효하다.
- 최종 패키지는 frozen product `c0baf4c`, review evidence `4ca4b48`, acceptance support `dcef304`를 구분해야 하며 GitHub checks는 정확한 support head에서 PASS해야 한다.
